	...
	<div>
		...
		<div class="container">
			<div class="handle-box">
				<el-select placeholder="请选择" v-model="pageInfo.index" class="handle-select mr10">
					<el-option key="1" label="客户名" value="connectionName"></el-option>
					<el-option key="2" label="出库编号" value="delivery_order_no"></el-option>
			...
			<el-table :data="delivery" border class="table" ref="multipleTable" header-cell-class-name="table-header">
				...
				<el-table-column prop="deliveryOrderNo" label="出库单号"></el-table-column>
				...
				<el-table-column prop="state" label="出库状态">
					<template v-slot="scope">
						...
						<p v-if="scope.row.state==1">已出库</p>
					</template>
				</el-table-column>
				<el-table-column label="操作" width="180" align="center">
					<template #default="scope">
						<el-button type="text" @click="handleEdit(scope.row)">出库</el-button>
		...
		<el-dialog title="新增" v-model="dialogFormVisible" width="30%">
			<el-form label-width="70px">
				出库编号:<el-input v-model="from1.deliveryOrderNo" placeholder="出库编号"></el-input>
				选择销售单:<el-select v-model="from1.said" placeholder="选择销售单" @change="Sale_Sid" style="width:420px;">
					<el-option v-for="itms in Sale" :label="itms.salesOrderNo" :value="itms.said"></el-option>
				</el-select>
				客户名称:<el-input v-model="cname" disabled placeholder="客户名称"></el-input>
				商品名称:<el-input v-model="sname" disabled placeholder="商品名称"></el-input>
		...
		<el-dialog title="编辑" v-model="dialogFormVisible2" width="30%">
			<el-form label-width="70px">
				出库编号:<el-input v-model="from1.deliveryOrderNo" placeholder="出库编号"></el-input>
				选择销售单:<el-select v-model="from1.said" placeholder="选择销售单" @change="Sale_Sid" style="width:420px;">
					<el-option v-for="itms in Sale" :label="itms.salesOrderNo" :value="itms.said"></el-option>
			...
			<template #footer>
				<span class="dialog-footer">
					...
					<el-button type="primary" @click="updateDelS()">确 定</el-button>
				</span>
			</template>
		</el-dialog>
	</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			handleEdit(row) {
				const _this = this
				this.axios.put("http://localhost:8089/m1/chuku/" + row.did + "/" + row.state)
					.then(res => {
						if (res.data.data == 1) {
							_this.$message({
								message: '出库成功',
			...
			handleDelete(row) {
				const _this = this
				this.axios.delete("http://localhost:8089/m1/delDelverid/" + row.did + "/" + row.state+"/"+row.said)
					.then(res => {
			...
			updateDelS() {
				const _this = this
				this.axios.put("http://localhost:8089/m1/delDelverid", this.from1)
					.then(res => {
						if (res.data.data == 1) {
							_this.$message({
								message: '修改成功',
			...
			UpdateDelvery(row) {
				...
				this.from1.said2 = row.said
				var row2 = this.Shop.filter(s => s.sid == row.sid)[0]
				this.sname = row2.sname
				var row3 = this.Conn.filter(c => c.conId == row.conId)[0]
				this.cname = row3.connectionName
				this.from1.did = row.did
				this.from1.sid = row.sid
				this.from1.said = row.said
				...
				this.from1.deliveryOrderNo = row.deliveryOrderNo
			...
			Sale_Sid() {
				this.from1.deliveryOrderNo = 'CK' + this.getProjectNum() + Math.floor(Math.random() * 10000)
				var row = this.Sale.filter(s => s.said == this.from1.said)[0]
			...
			AddDelvery() {
				const _this = this
				this.axios.post("http://localhost:8089/m1/AddDelverid", this.from1)
					.then(function(response) {
						console.log("AddDelverid?????")
